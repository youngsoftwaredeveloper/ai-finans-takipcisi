    ...
    const analyzeExpenses = () => {
        ...
        if (coffeeCount >= 3) {
            insights.push({
                ...
                title: "Kahve Alışkanlığı Analizi",
                message: `Bu ay ${coffeeCount} kez kahve/içecek harcaması yapmışsın (Toplam: ₺${coffeeTotal.toFixed(2)}). Çok fazla dışarıda kahve içiyorsun gibi görünüyor.`,
                tip: `☕ Evde demlemeye başlarsan bu ay yaklaşık ₺${(coffeeTotal * 0.8).toFixed(2)} (%20) tasarruf edebilirsin!`,
        ...
        if (totalAmount > 0) {
            ...
            insights.push({
                ...
                title: "Genel Tasarruf Potansiyeli",
                message: `Harcamalarını analiz ettim. Bazı esnek harcamalarından kısarak bu ay ₺${potentialSavings.toFixed(2)} ek birikim yapabilirsin.`,
                tip: "💡 Haftalık bütçe limitleri belirleyerek harcamalarını %15 oranında optimize edebilirim.",
        ...
        if (maxAmt > (totalAmount * 0.4)) {
            insights.push({
                ...
                message: `${mainCategory} harcamaların toplam bütçenin %40'ından fazlasını oluşturuyor!`,
                tip: "📊 Diğer kategorilere yer açmak için bu kalemi gözden geçirmelisin.",
        ...
        <div className="glass rounded-2xl p-6 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-bold text-foreground">AI Finans Asistanı</h3>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-semibold">Aktif Analiz</p>
                    </div>
                </div>
                {isAnalyzing && (
                ...
                {advice.length === 0 ? (
                    <div className="text-center py-4">
                        ...
                        <p className="text-sm text-muted-foreground italic">
                            Harcama verilerini bekliyorum. Henüz analiz edecek bir şey bulamadım...
                        </p>
                    </div>
                ) : (
                    advice.map((item, idx) => (
            ...
            <div className="mt-6 pt-4 border-t border-border/50 text-center">
                <button className="text-xs text-primary hover:underline font-medium">
                    Daha fazla detaylı analiz göster →
                </button>
            </div>
        </div>
    );
